                    ...
                    <div class="panel" id="print-panel" style="min-height: 842px">
                        <header class="panel-heading">
                            <h3 class="panel-title">
                                <strong>Performance Loss Analysis - Next</strong>
                                @if($shift[0] == 'All-Day')
                        ...
                        <div class="panel-body">
                            ...
                            <table class="table table-hover dataTable table-striped w-full" id="production-report-table">
                                ...
                                <tr>
                                    ...
                                    <th style="text-align: center">Design\Sleeve Speed</th>
                                ...
                                @foreach($records as $row)
                                    @if(!in_array($row['id'],$alreadyDoneMaterials))
                                        <tr  onclick="hiderow({{$row['id']}})" style="background-color: lightgrey; color: black; ">
                                            <td colspan="3" id="materialProduction" data-material-production="{{$row['materialProduction']}}" data-material-name="{{$row['material_combination']}}">Substrate: <strong>{{$row['material_combination']}}</strong></td>
                                            ...
                                            <td class="text-center" colspan="1">
                                                <strong>
                                                    @if($machine->time_uom == 'Min')
                                                        {{number_format(($row['materialTargetHour']/$row['materialRunTime'])*100,0)}}
                                                    @endif
                                                    @if($machine->time_uom == 'Hr')
                                                        {{number_format(((($row['materialTargetHour']/$row['materialRunTime'])*60)*100),0)}}
                                                    @endif
                                    ...
                                    @if(!in_array($row['product_number'],$alreadyDoneProducts))
                                        <tr onclick="hiderow({{$row['product_number']}})" class = "{{$row['id']}} tableexport-ignore" style="display: none; background-color: #302e2e; color: #ffffff ">
                                            ...
                                            <td style="color:red">{{$row['product_sleeve_speed']}}</td>
                                            ...
                                            <td style="text-align: center">
                                                @if($machine->time_uom == 'Min')
                                                    {{$row['productProduction'] == 0?0:number_format(($row['productProduction']/$row['productRunTime'])/(isset($row['product_sleeve_speed'])?$row['product_sleeve_speed']:$machine->max_speed)*100,0)}}
                                                @endif
                                                @if($machine->time_uom == 'Hr')
                                                    {{$row['productProduction'] == 0?0:number_format((($row['productProduction']/$row['productRunTime'])*60)/(isset($row['product_sleeve_speed'])?$row['product_sleeve_speed']:$machine->max_speed)*100,0)}}
                                                @endif
                                                @if($machine->time_uom == 'Sec')
                                                    {{$row['productProduction'] == 0?0:number_format((($row['productProduction']/$row['productRunTime'])/60)/(isset($row['product_sleeve_speed'])?$row['product_sleeve_speed']:$machine->max_speed)*100,0)}}
                                            ...
                                            <td style="text-align: center">
                                                @if($machine->time_uom == 'Min')
                                                    {{$row['productProduction'] == 0?0:100-number_format(($row['productProduction']/$row['productRunTime'])/(isset($row['product_sleeve_speed'])?$row['product_sleeve_speed']:$machine->max_speed)*100,0)}}
                                                @endif
                                                @if($machine->time_uom == 'Hr')
                                                    {{$row['productProduction'] == 0?0:100-number_format((($row['productProduction']/$row['productRunTime'])*60)/(isset($row['product_sleeve_speed'])?$row['product_sleeve_speed']:$machine->max_speed)*100,0)}}
                                                @endif
                                                @if($machine->time_uom == 'Sec')
                                                    {{$row['productProduction'] == 0?0:100-number_format((($row['productProduction']/$row['productRunTime'])/60)/(isset($row['product_sleeve_speed'])?$row['product_sleeve_speed']:$machine->max_speed)*100,0)}}
                                                @endif
                                            </td>
                                            <td style="text-align: center">{{$row['productProduction'] == 0 || $row['nominal_speed'] == 0?0:number_format(($row['productProduction']/$row['productRunTime'])/$row['nominal_speed']*100,0)}}</td>
                                            <td style="text-align: center">{{$row['productProduction'] == 0 || $row['nominal_speed'] == 0?0:100 - number_format(($row['productProduction']/$row['productRunTime'])/$row['nominal_speed']*100,0)}}</td>
                                        </tr>
                            ...
                            <div class="row col-12">
                                ...
                                <div class="col-2">
                                    ...
                                    <strong>
                                        @if($machine->time_uom == 'Min')
                                            {{number_format((($totalRunTime>0?($totalTargetHour/$totalRunTime):0)*100), 2)}} %
                                        @endif
                                        @if($machine->time_uom == 'Hr')
                                            {{number_format((($runTime>0?($totalTargetHour/$totalRunTime)*60:0)*100), 2)}} %
                                        @endif
                                        @if($machine->time_uom == 'Sec')
                                            {{number_format((($runTime>0?($totalTargetHour/$totalRunTime)/60:0)*100), 2)}} %
                                        @endif
