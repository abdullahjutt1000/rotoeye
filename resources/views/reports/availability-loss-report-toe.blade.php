...
@section('body')
    <div class="page">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-4">
                                                        <div> <select class="form-control lossesShiftSelection"
                                                                ...
                                                                @foreach ($machine->section->department->businessUnit->shifts as $shiftts)
                                            ...
                                            @if ($row['errDuration'] < 10)
                                                <tr id="errorHistory">
                                                    ...
                                                    <td style="text-align: center">
                                                        {{ number_format(($row['errDuration'] / $budgetedTime) * 100, 0) }}%
                                                    </td>
                                                </tr>
                                            @endif
                                        @endforeach
                                    @else
                                        @foreach ($graphRecords as $row)
                                            <tr id="errorHistory">
                                                ...
                                                <td style="text-align: center">
                                                    {{ number_format(($row['errDuration'] / $budgetedTime) * 100, 0) }}%
                                                    {{-- {{ number_format(round(($row['errDuration'] / $budgetedTime) * 100), 0) }}% --}}
        ...
        {{--   Availability Loss Report (EE) start --}}
        <div class="page-content">
            <div class="row" data-plugin="matchHeight" data-by-row="true">
                <div class="col-md-12">
                    <div class="panel" id="print-panel">
                                    ...
                                    @if ($report_type == 'short_stops')
                                        @foreach ($graphRecords as $row)
                                            @if (
                                                ...
                                                <tr id="errorHistory">
                                                    <td id="error_id" data-error-id="{{ $row['error_id'] }}">
                                                        <a
                                                            href="{{ URL::to('get/error/history' . '/' . $machine->id . '/' . date('Y-m-d', strtotime($from)) . '/' . date('Y-m-d', strtotime($to)) . '/' . $row['error_id'] . '/' . serialize($shift)) }}">
                                                            {{ $row['error_id'] }}
                                                        </a>
                                                    </td>
                                                    <td id="error_name" data-error-name="{{ $row['error_name'] }}">
                                                    ...
                                                    <td id="duration" data-error-duration="{{ $row['errDuration'] }}">
                                                        {{ number_format($row['errDuration'], 2) }}
                                                    </td>
                                                    <td id="frequency" data-error-frequency="{{ $row['frequency'] }}">
                                                        {{ $row['frequency'] }}
                                                    </td>
        ...
        {{--   Availability Loss Report (EE) end --}}

        {{--   Availability Loss Report (OEE) start --}}
        <div class="page-content">
            <div class="row" data-plugin="matchHeight" data-by-row="true">
                <div class="col-md-12">
                    <div class="panel" id="print-panel">
                        ...
                        <div class="panel-body">
                            <table class="table table-hover dataTable table-striped w-full" id="production-report-table2">
                                ...
                                <tbody>
                                    ...
                                    @if ($report_type == 'short_stops')
                                        @foreach ($graphRecords as $row)
                                            @if ($row['errDuration'] < 10 && $row['error_toe_category'] !== 'Strategic Losses/Supply Chain Losses')
                                                <tr id="errorHistory">
                                                    <td id="error_id" data-error-id="{{ $row['error_id'] }}">
                                                        <a
                                                            href="{{ URL::to('get/error/history' . '/' . $machine->id . '/' . date('Y-m-d', strtotime($from)) . '/' . date('Y-m-d', strtotime($to)) . '/' . $row['error_id'] . '/' . serialize($shift)) }}">
                                    ...
                                    @else
                                        @foreach ($graphRecords as $row)
                                            @if ($row['error_toe_category'] !== 'Strategic Losses/Supply Chain Losses')
                                                <tr id="errorHistory">
                                                    <td id="error_id" data-error-id="{{ $row['error_id'] }}">
                                                        <a
                                                            href="{{ URL::to('get/error/history' . '/' . $machine->id . '/' . date('Y-m-d', strtotime($from)) . '/' . date('Y-m-d', strtotime($to)) . '/' . $row['error_id'] . '/' . serialize($shift)) }}">
        ...
        {{--   Availability Loss Report (OEE) end --}}
...
@section('footer')
    ...
    {{-- Chart Code for the availability-loss-report-toe start  --}}
    ...
    {{-- Chart Code for the availability-loss-report-toe end  --}}

    {{-- Chart Code for the availability-loss-report-ee start  --}}
    <script>
        const data2 = {
            labels: [],
            datasets: [{
    ...
    {{-- Chart Code for the availability-loss-report-ee end  --}}
