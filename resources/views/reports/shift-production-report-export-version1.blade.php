    ...
    <div class="page" style='display:none'>
        <div class="page-header">
            <div class="page-header-actions" style="left: 30px">
                <button onclick="exportTableToExcel('production-report-table', '{{$machine->sap_code.'-'.$machine->name.' Shift Production Report'}}')" class="btn btn-sm btn-icon btn-success btn-round" data-toggle="tooltip" data-original-title="Export to Excel" id="down">Export to Excel</button>
            </div>
        </div>
...
@section('footer')
    ...
    <script>
        ...
        window.onload = function(e){ 
            exportTableToExcel('production-report-table', '{{$machine->sap_code.'-'.$machine->name.' Shift Production Report'}}')
            history.back();
        }
