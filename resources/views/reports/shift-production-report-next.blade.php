                    ...
                    <div class="panel" id="print-panel" style="min-height: 842px">
                        <header class="panel-heading">
                            <h3 class="panel-title">
                                <strong>Daily Shift Production Report-Next</strong> - <small>{{isset($from)?$from.' to '.$to:$date}}</small>
                                    ...
                                    @if(isset($records[$i-1]['job_id']))
                                        @if($records[$i]['job_id'] != $records[$i-1]['job_id'])
                                            ...
                                            <tr style="background-color:black; color: white">
                                                <td colspan="3">
                                                    <strong>No. of UP's: </strong>{{isset($records[$i]['product_ups'])?$records[$i]['product_ups']:'-'}}<br>
                                                    <strong>Trim Width (mm): </strong>{{isset($records[$i]['product_trim_width'])?$records[$i]['product_trim_width']*1000:'-'}}<br>
                                                    <strong>Slitted Reel Width (mm): </strong>{{isset($records[$i]['product_slitted_reel_width'])?$records[$i]['product_slitted_reel_width']*1000:'-'}}<br>
                                                    <strong>Possible Addition of UP's: </strong> @if(isset($machine->machine_width)&&isset($records[$i]['product_trim_width'])&&isset($records[$i]['product_slitted_reel_width'])&&isset($records[$i]['product_ups']))

                                                        {{number_format(($machine->machine_width-($records[$i]['product_trim_width']+($records[$i]['product_slitted_reel_width']*$records[$i]['product_ups'])))/$records[$i]['product_slitted_reel_width'],0)}}
                                                    @else
                                                        -
                                                    @endif
                                                    <br>
                                                ...
                                                <td colspan="6">
                                                    <strong>Thickness (mm): </strong>{{isset($records[$i]['product_thickness'])?$records[$i]['product_thickness']*1000:'-'}}<br>
                                                    <strong>Density: </strong>{{isset($records[$i]['product_density'])?$records[$i]['product_density']:'-'}}<br>
                                                    <strong>GSM(Kg's) : </strong>{{isset($records[$i]['product_gsm'])?$records[$i]['product_gsm']*1000:'-'}}<br>
                                                </td>
                                                <td colspan="2">
