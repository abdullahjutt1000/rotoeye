                    ...
                    <div class="panel" id="print-panel">
                        <header class="panel-heading">
                            <h3 class="panel-title">
                                <strong>{{$error->name}} History</strong> - <small>{{isset($from)?date('d M, Y', strtotime($from)).' to '.date('d M, Y', strtotime($to)):$date}}</small><br>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="form-group row">
                                                <div class="col-sm-10">
                                                    <input type="text" readonly class="form-control-plaintext text-secondary from" name="downtime_record_form[]" value="{{date('Y-m-d H:i:s', strtotime($rec['from']))}}" hidden height="0px">
                                        ...
                                        <td>
                                            <div class="form-group row">
                                                <div class="col-sm-10">
                                                    <input type="text" readonly class="form-control-plaintext text-secondary to" name="downtime_record_to[]" value="{{date('Y-M-d H:i:s', strtotime($rec['to']))}}" height="0px" hidden>
                                        ...
                                        <td>
                                            ...
                                            <select class="form-control form-control-sm error_id" data-plugin="select2"  aria-label="Default select example" name="downtime_record_error_id[]">
                                                @foreach($errors as $error)
                                                    @if($error->id==$rec['error_id'])
                                                    <option selected value="{{$error->id}}">{{$error->id}} - {{$rec['error_name']}}</option>
                                                    @endif
                                                    <option value="{{$error->id}}">{{$error->id}} - {{$error->name}}</option>
                                                @endforeach
                                            </select>
                                        </td>

                                        <td ><textarea name="downtime_record_err_comments[]" class="form-control err_comments" id="exampleFormControlTextarea1" rows="1">{{$rec['err_comments']}}</textarea></td>
                                        <td>{{number_format($rec['duration'],0)}}</td>
                                        @if($rec['error_id']!=500)
                                            <td><button  onclick="allocate_downtime($(this))" class="btn btn-success">Save</button></td>
                                        @else
                                            <td><button  onclick="allocate_downtime($(this))" class="btn btn-danger">Save</button></td>
                                        @endif
                ...
                $.ajax({
                    ...
                    statusCode: {
                        200: function (response) {
                            ...
                            if(rec.parent().parent().get( 0 ).children[2].getElementsByTagName('select')[0].value != 500)
                            {
                        ...
                        500: function (response) {
                           ...
                           //old alert('Downtime cannot exceed 24 Hours. Please check date and time.');
                           alert('Downtime cannot exceed 48 Hours. Please check date and time.');
                        }
                    }
                });
