...
class ImportGroupDashboardDataInChunks extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cron:importgroupdashboarddatachunks';
    ...
    {
        try {
            Log::info("======================Cron Job Started in chunks for groupdashboard(" . date('Y-m-d H:i:s') . ")========================");
            $machinesIds = [5,6,7,8,9];
            // $machinesIds = [10,11,13,14,15];
            // $machinesIds = [16,17,18,19,21];
            // $machinesIds = [22,23,24,25,26];
            // $machinesIds = [27,28,29,30,31];
            // $machinesIds = [32,33,34,35,36];
            // $machinesIds = [37,38,39,40,41];
            // $machinesIds = [42,43,44,45,46];
            // $machinesIds = [47,48,49,50,51];
            // $machinesIds = [52,53,54,55,56];
            // $machinesIds = [57,58,59,60,61];
            // $machinesIds = [62,63,64,65,66];
            // $machinesIds = [67,68,69,70,71];
            // $machinesIds = [72,73,74,75,76];
            // $machinesIds = [77,78,79,80,81];
            // $machinesIds = [82,83,84,85,86];
            // $machinesIds = [87,88,89,90,91];
             
           
           
           
           
            $machines = Machine::whereIn("id",$machinesIds)->whereNull('is_disabled')->orderBy("sd_status","ASC")->get();
            $data['machines'] = [];
            $data['records'] = [];
            ...
            foreach ($machines as $machine) {
                ...
                $logger->log('--------------------------------------------- CRON JOB START in chunks for  groupdashboard ---------------------------------------------', $machine);
               
                $count = 0;
                try {
                    $currentDate = date("Y-m-d");
                        $date = date("2023-01-01"); //date('Y-m-d');
                        ...
                        $records = DB::table('records')
                            ...
                            if(count($records) > 0){
                             ...
                             // dd($data['endDateTime']);
                                ...
                                Log::info("Cron for in chunks  groupdashboard Started(" . date('Y-m-d H:i:s') . "): " . $machine->name);
                            }
